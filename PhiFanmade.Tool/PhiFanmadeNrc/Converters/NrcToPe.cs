...
public static class NrcToPe
{
    ...
    /// <summary>
    /// 将 NRC 缓动编号映射到 PE 缓动编号。
    /// </summary>
    /// <param name="nrcEasing">NRC 缓动编号。</param>
    /// <returns>PE 缓动编号。</returns>
    private static int MapEasingNumber(int nrcEasing)
    {
    ...
    private static void ConvertAlphaEvents(Pe.JudgeLine target, List<Nrc.Event<int>>? sourceEvents)
    {
        ...
        var ordered = new List<Nrc.Event<int>>();
        foreach (var srcEvent in sourceEvents.OrderBy(e => (double)e.StartBeat))
        {
            ...
            ordered.AddRange(NrcEventTools.EventListCompress(sliced));
        }
        ...
        string channelName)
    {
        ...
        var ordered = new List<Nrc.Event<T>>();
        foreach (var srcEvent in sourceEvents.OrderBy(e => (double)e.StartBeat))
        {
            ordered.AddRange(ExpandUnsupportedEasing(srcEvent, $"{channelName}@{(double)srcEvent.StartBeat:F3}"));
        }
