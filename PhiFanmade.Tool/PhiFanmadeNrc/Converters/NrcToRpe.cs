...
public static class NrcToRpe
{
    ...
    [PublicAPI]
    public sealed class EasingNotSupportedException : Exception
    {
        public EasingNotSupportedException(int nrcEasing)
            : base($"NRC 缓动编号 {nrcEasing} 在 RPE 中无对应项，需切割事件拟合") { }
    }
    ...
    private static Rpe.EventLayer ConvertEventLayer(Nrc.EventLayer src)
    {
        ...
        if (src.MoveXEvents  != null) { rpe.MoveXEvents  = []; foreach (var e in src.MoveXEvents)  rpe.MoveXEvents.AddRange(ConvertFloatEventExpanding(e, TransformX)); }
        if (src.MoveYEvents  != null) { rpe.MoveYEvents  = []; foreach (var e in src.MoveYEvents)  rpe.MoveYEvents.AddRange(ConvertFloatEventExpanding(e, TransformY)); }
        if (src.RotateEvents != null) { rpe.RotateEvents = []; foreach (var e in src.RotateEvents) rpe.RotateEvents.AddRange(ConvertFloatEventExpanding(e, TransformAngle)); }
        if (src.AlphaEvents  != null) { rpe.AlphaEvents  = []; foreach (var e in src.AlphaEvents)  rpe.AlphaEvents.AddRange(ConvertIntEventExpanding(e)); }
        if (src.SpeedEvents  != null) { rpe.SpeedEvents  = []; foreach (var e in src.SpeedEvents)  rpe.SpeedEvents.AddRange(ConvertFloatEventExpanding(e)); }
        return rpe;
    }
    ...
    private static Rpe.ExtendLayer ConvertExtendLayer(Nrc.ExtendLayer src)
    {
        ...
        if (src.ScaleXEvents != null) { rpe.ScaleXEvents = []; foreach (var e in src.ScaleXEvents) rpe.ScaleXEvents.AddRange(ConvertFloatEventExpanding(e)); }
        if (src.ScaleYEvents != null) { rpe.ScaleYEvents = []; foreach (var e in src.ScaleYEvents) rpe.ScaleYEvents.AddRange(ConvertFloatEventExpanding(e)); }
        if (src.TextEvents   != null) rpe.TextEvents   = src.TextEvents.ConvertAll(ConvertStringEvent);
        if (src.PaintEvents  != null) { rpe.PaintEvents  = []; foreach (var e in src.PaintEvents)  rpe.PaintEvents.AddRange(ConvertFloatEventExpanding(e)); }
        if (src.GifEvents    != null) { rpe.GifEvents    = []; foreach (var e in src.GifEvents)    rpe.GifEvents.AddRange(ConvertFloatEventExpanding(e)); }
        return rpe;
    }
    ...
    private static List<Rpe.Event<float>> ConvertFloatEventExpanding(
        Nrc.Event<float> src, Func<float, float> valueTransformer = null)
    {
        valueTransformer ??= v => v;
        try   { return [ConvertEvent(src, valueTransformer: valueTransformer)]; }
        catch (EasingNotSupportedException) { return CutEventToLinear(src, valueTransformer); }
    }

    private static List<Rpe.Event<float>> CutEventToLinear(
        Nrc.Event<float> src, Func<float, float> valueTransformer)
    {
        ...
        for (var i = 0; i < UnsupportedEasingCutPrecision; i++)
        {
            ...
            var next   = isLast ? src.EndBeat : new Beat((double)src.StartBeat + (i + 1.0) / UnsupportedEasingCutPrecision * totalBeats);
    ...
    {
        try   { return [ConvertEvent(src)]; }
        catch (EasingNotSupportedException) { return CutEventToLinearInt(src); }
    }
    ...
    private static Rpe.Event<byte[]>  ConvertByteArrayEvent(Nrc.Event<byte[]> src) => ConvertEvent(src, v => v?.ToArray() ?? []);
    private static Rpe.XControl       ConvertXControl(Nrc.XControl src)       => new() { Easing = ConvertEasing(src.Easing), X = src.X, Pos  = src.Pos  };
    private static Rpe.AlphaControl   ConvertAlphaControl(Nrc.AlphaControl src) => new() { Easing = ConvertEasing(src.Easing), X = src.X, Alpha = src.Alpha };
    private static Rpe.SizeControl    ConvertSizeControl(Nrc.SizeControl src)   => new() { Easing = ConvertEasing(src.Easing), X = src.X, Size  = src.Size  };
    private static Rpe.SkewControl    ConvertSkewControl(Nrc.SkewControl src)   => new() { Easing = ConvertEasing(src.Easing), X = src.X, Skew  = src.Skew  };
    private static Rpe.YControl       ConvertYControl(Nrc.YControl src)         => new() { Easing = ConvertEasing(src.Easing), X = src.X, Y     = src.Y     };
}
