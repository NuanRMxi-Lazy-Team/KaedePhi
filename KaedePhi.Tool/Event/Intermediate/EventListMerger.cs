...
public class EventListMerger<TPayload> : LoggableBase, IEventListMerger<IrEvents.Event<TPayload>>
    where TPayload : notnull
{
    ...
    /// <inheritdoc/>
    public List<IrEvents.Event<TPayload>> EventListMerge(
        List<IrEvents.Event<TPayload>>? toEvents,
        List<IrEvents.Event<TPayload>>? fromEvents,
        double precision
    )
    {
    ...
    protected static bool TryGetMergeEarlyReturn(
        List<IrEvents.Event<TPayload>>? toEvents,
        List<IrEvents.Event<TPayload>>? fromEvents,
        out List<IrEvents.Event<TPayload>> result
    )
    {
    ...
    protected static List<IrEvents.Event<TPayload>> CloneEventList(
    ...
    protected static void SortByStartBeat(List<IrEvents.Event<TPayload>> events)
    {
    ...
    protected static List<IrEvents.Event<TPayload>> MergeWithoutOverlap(
        List<IrEvents.Event<TPayload>> toEventsCopy,
        List<IrEvents.Event<TPayload>> fromEventsCopy
    )
    {
        var newEvents = (
            ...
            select new IrEvents.Event<TPayload>
            {
    ...
    private static bool TryGetOverlapBounds(
        IrEvents.Event<TPayload> fe,
        IrEvents.Event<TPayload> te,
        out Beat start,
        out Beat end
    )
    {
    ...
    /// <returns>合并后的事件列表。</returns>
    private static List<IrEvents.Event<TPayload>> MergeWithOverlapFixedSampling(
        List<IrEvents.Event<TPayload>> toEventsForOffsetLookup,
        List<IrEvents.Event<TPayload>> toEventsCopy,
        List<IrEvents.Event<TPayload>> fromEventsCopy,
        double precision
    )
    ...
    protected static List<IrEvents.Event<TPayload>> BuildBaseEventsOutsideOverlap(
        List<IrEvents.Event<TPayload>> toEventsCopy,
        List<IrEvents.Event<TPayload>> fromEventsCopy,
        List<IrEvents.Event<TPayload>> toEventsForOffsetLookup,
        List<(Beat Start, Beat End)> overlapIntervals
    ...
    {
        var newEvents = new List<IrEvents.Event<TPayload>>();
                ...
                foreach (var (gapStart, gapEnd) in GapsOutsideOverlap(toEvent))
                {
                    ...
                    newEvents.Add(
                        new IrEvents.Event<TPayload>
                        {
                            ...
                            Easing = new Ir.Easing(1), // 线性，具体看上方注释
                            IsBezier = false,
                        }
                    );
                }
        ...
        bool TouchesAnyOverlap(IrEvents.Event<TPayload> evt) =>
        ...
        List<(Beat Start, Beat End)> GapsOutsideOverlap(IrEvents.Event<TPayload> evt)
        {
    ...
    private static (
        List<IrEvents.Event<TPayload>> CutTo,
        List<IrEvents.Event<TPayload>> CutFrom
    ...
    {
        var cutTo = new List<IrEvents.Event<TPayload>>();
        var cutFrom = new List<IrEvents.Event<TPayload>>();
    ...
    private static List<IrEvents.Event<TPayload>> MergeCutOverlapSegments(
        List<IrEvents.Event<TPayload>> toEventsForOffsetLookup,
        List<IrEvents.Event<TPayload>> fromEventsCopy,
        List<IrEvents.Event<TPayload>> cutTo,
        List<IrEvents.Event<TPayload>> cutFrom,
    ...
    {
        var allCutEvents = new List<IrEvents.Event<TPayload>>();
        foreach (var (start, end) in overlapIntervals)
        {
    ...
    private static List<IrEvents.Event<TPayload>> MergeSingleOverlapInterval(
        List<IrEvents.Event<TPayload>> cutTo,
        List<IrEvents.Event<TPayload>> cutFrom,
        Beat start,
        Beat end,
    ...
    )
    {
        ...
        var toIndex = new Dictionary<(double, double), IrEvents.Event<TPayload>>();
        foreach (var e in cutTo)
            toIndex.TryAdd((e.StartBeat, e.EndBeat), e);
        var fromIndex = new Dictionary<(double, double), IrEvents.Event<TPayload>>();
        foreach (var e in cutFrom)
            fromIndex.TryAdd((e.StartBeat, e.EndBeat), e);

        var merged = new List<IrEvents.Event<TPayload>>();
