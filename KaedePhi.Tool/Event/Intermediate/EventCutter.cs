...
/// <summary>
/// IR 事件切割器：将事件列表按指定拍长切割为等长段。
/// </summary>
public class EventCutter<TPayload> : LoggableBase, IEventCutter<IrEvents.Event<TPayload>, Beat>
    where TPayload : notnull
{
    /// <inheritdoc/>
    public List<IrEvents.Event<TPayload>> CutEventsInRange(
    ...
    )
    {
        ...
        var cutEvents = new List<IrEvents.Event<TPayload>>();
    ...
    /// <inheritdoc/>
    public List<IrEvents.Event<TPayload>> CutEventToLinear(
        IrEvents.Event<TPayload> evt,
        double cutLength
