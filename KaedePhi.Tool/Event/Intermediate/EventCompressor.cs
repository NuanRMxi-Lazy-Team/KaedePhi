...
/// <summary>
/// IR 事件压缩器：合并变化率相近的相邻线性事件，以及移除无意义的默认值事件。
/// </summary>
public class EventCompressor<TPayload> : LoggableBase, IEventCompressor<IrEvents.Event<TPayload>>
    where TPayload : notnull
{
    ...
    private static bool TryMergeSqrt(
        IrEvents.Event<TPayload> last,
        IrEvents.Event<TPayload> cur,
        double relTol
    )
    {
    ...
    /// 判断事件是否为可安全压缩的完整线性事件。
    /// </summary>
    private static bool IsEligibleForTransform(IrEvents.Event<TPayload> evt)
    {
        return !evt.IsBezier && (int)evt.Easing == 1 && evt.Font is null;
    }

    /// <inheritdoc/>
    public List<IrEvents.Event<TPayload>> EventListCompressSqrt(
    ...
    )
    {
        ...
        var compressed = new List<IrEvents.Event<TPayload>> { events[0].Clone() };
    ...
    /// <inheritdoc/>
    public List<IrEvents.Event<TPayload>> EventListCompressSlope(
        List<IrEvents.Event<TPayload>>? events,
        double tolerance,
        IProgress<ToolProgress>? progress = null
    )
