    ...
    where TPayload : notnull
{
    ...
    /// </summary>
    private List<IrEvents.Event<TPayload>> MergeWithOverlapAdaptiveSampling(
    ...
    /// 逐个重叠区间执行自适应合并。
    /// </summary>
    private List<IrEvents.Event<TPayload>> MergeAdaptiveIntervals(
        List<IrEvents.Event<TPayload>> toEventsCopy,
        List<IrEvents.Event<TPayload>> fromEventsCopy,
    ...
    )
    {
        ...
        var result = new List<IrEvents.Event<TPayload>>();
    ...
    /// </para>
    /// </summary>
    private List<IrEvents.Event<TPayload>> MergeAdaptiveSingleInterval(
        List<IrEvents.Event<TPayload>> toEventsCopy,
        List<IrEvents.Event<TPayload>> fromEventsCopy,
        Beat start,
        Beat end,
    ...
    /// </summary>
    private List<IrEvents.Event<TPayload>> AdaptiveSampleSubInterval(
        List<IrEvents.Event<TPayload>> toEventsCopy,
        List<IrEvents.Event<TPayload>> fromEventsCopy,
        Beat subStart,
        Beat subEnd,
    ...
    /// </summary>
    private static IrEvents.Event<TPayload>? GetActiveEventAtBeat(
    ...
    )
    {
        ...
        IrEvents.Event<TPayload>? result = null;
    ...
    private static TPayload? GetPreviousEndValue(List<IrEvents.Event<TPayload>> events, Beat beat)
    {
    ...
    private static (TPayload? Outgoing, TPayload? Incoming) GetNextBeatValues(
        List<IrEvents.Event<TPayload>> events,
        IrEvents.Event<TPayload>? eventAtCurrent,
        IrEvents.Event<TPayload>? eventAtNext,
        Beat nextBeat
    )
    {
    ...
    private static void AddSegmentEvent(
        List<IrEvents.Event<TPayload>> target,
        Beat startBeat,
        Beat endBeat,
