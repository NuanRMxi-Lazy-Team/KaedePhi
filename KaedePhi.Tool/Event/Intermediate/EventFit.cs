...
/// </summary>
public class EventFit<TPayload> : LoggableBase, IEventFit<IrEvents.Event<TPayload>>
    where TPayload : notnull
{
    ...
    /// 常量事件和非线性事件直接原样写入输出。
    /// </summary>
    private static List<IrEvents.Event<TPayload>> FitEventsCore(
        List<IrEvents.Event<TPayload>> sortedEvents,
        double tolerance
    )
    {
    ...
    /// </summary>
    private static int CollectRunEnd(List<IrEvents.Event<TPayload>> events, int start)
    {
    ...
    private static void FitRunInto(
        List<IrEvents.Event<TPayload>> source,
        int runStart,
        int runEnd,
    ...
    /// </summary>
    private static double GetEasingValue(Ir.Easing easing, double t)
    {
        return Ir.Easings.Evaluate(easing, 0d, 1d, t);
    }
    ...
    private static bool IsContiguous(
        IrEvents.Event<TPayload> first,
        IrEvents.Event<TPayload> second
    )
    {
    ...
    /// 判断事件起止值是否相同（常量事件）。
    /// </summary>
    private static bool IsNumericConstant(IrEvents.Event<TPayload> evt)
    {
    ...
    /// 判断事件是否使用可参与拟合的线性缓动。
    /// </summary>
    private static bool IsLinear(IrEvents.Event<TPayload> evt)
    {
    ...
    /// </summary>
    private static IrEvents.Event<TPayload> CreateMergedEvent(
        IrEvents.Event<TPayload> template,
        Beat startBeat,
        double startValue,
        Beat endBeat,
        double endValue,
    ...
    {
        var evt = new IrEvents.Event<TPayload>
        {
            StartBeat = new Beat((int[])startBeat),
            EndBeat = new Beat((int[])endBeat),
            Easing = new Ir.Easing(easingId),
            Font = template.Font,
        };
