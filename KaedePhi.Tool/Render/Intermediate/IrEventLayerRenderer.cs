...
public static class IrEventLayerRenderer
{
    ...
    public static SKBitmap RenderEventLayer(EventLayer layer, IrRenderOptions opts)
    {
        ArgumentNullException.ThrowIfNull(layer);
        IrRenderValidator.ValidateEventLayer(layer, opts);
        ...
        const int numCh = 5;
        var totalWidth =
            opts.LeftMargin + numCh * opts.ChannelWidth + (numCh - 1) * opts.ChannelPadding + 8;
        var totalHeight =
            opts.HeaderHeight
            + (int)Math.Ceiling(totalBeats * opts.PixelsPerBeat)
            + opts.BottomPadding;
    ...
    private static List<ChannelData> BuildChannels(EventLayer layer, IrRenderOptions opts)
    {
        return
        [
            BuildChannel(
                ...
                opts.MoveXColor,
                layer.MoveXEvents,
                (e, b) => e.GetValueAtBeatAsDouble(b),
                opts
            ),
            BuildChannel(
                "MoveY",
    ...
    private static ChannelData BuildChannel<T>(
        ...
        Func<IrEvents.Event<T>, Beat, double> getValue,
        IrRenderOptions opts
    )
        where T : notnull
    {
        ...
        where T : notnull
    {
        ...
        {
            double c = (mn + mx) / 2.0,
                half = Math.Max(Math.Abs(c) * opts.MinValueRangeHalfRatio, opts.MinValueRangeHalf);
    ...
    private static double ComputeTotalBeats(EventLayer layer)
    {
        ...
        void Scan<T>(List<IrEvents.Event<T>>? list)
            where T : notnull
        {
            if (list == null)
                return;
    ...
    private static float BeatToY(double beat, IrRenderOptions opts, int totalHeight) =>
    ...
    {
        if (maxVal <= minVal)
            return channelX + channelWidth / 2f;
        return channelX
            + (float)(Math.Clamp((value - minVal) / (maxVal - minVal), 0.0, 1.0) * channelWidth);
    }
    ...
    )
    {
        ...
        var group = new List<(double Start, double End, Func<double, double> GetValue)>
        {
            events[0],
        };
    ...
    private static Segment BuildSegment(
        List<(double Start, double End, Func<double, double> GetValue)> group,
        IrRenderOptions opts
    )
    {
        double epMn = double.MaxValue,
            epMx = double.MinValue;
        double smMn = double.MaxValue,
            smMx = double.MinValue;
    ...
    )
    {
        ...
        for (double b = 0; b <= totalBeats + step * 0.01; b += step)
        {
            ...
            if (y > totalHeight - opts.HeaderHeight)
                continue;
            canvas.DrawLine(0, y, totalWidth, y, isWholeBeat ? beatPaint : subPaint);
        }
    }

    private static void DrawBeatLabels(
        SKCanvas canvas,
        double totalBeats,
        int totalHeight,
        IrRenderOptions opts
    ...
    private static void DrawChannelBackground(
        SKCanvas canvas,
        int channelX,
        int totalHeight,
        IrRenderOptions opts
    )
    {
        ...
        canvas.DrawRect(
            SKRect.Create(channelX, 0, opts.ChannelWidth, totalHeight - opts.HeaderHeight),
            bgPaint
        );
    }

    private static void DrawChannelHeader(
        SKCanvas canvas,
        int channelX,
        ChannelData ch,
        IrRenderOptions opts,
    ...
    )
    {
        ...
        canvas.DrawRect(
            SKRect.Create(channelX, footerTop, opts.ChannelWidth, opts.HeaderHeight),
            hdrBg
        );
    ...
    private static void DrawChannelContent(
        SKCanvas canvas,
        int channelX,
        ChannelData ch,
        int totalHeight,
    ...
    )
    {
        ...
        foreach (var seg in segments)
        {
            foreach (var (start, end, getValue) in seg.Events)
            {
                ...
                canvas.DrawRect(
                    SKRect.Create(channelX, yTop, opts.ChannelWidth, blockH),
                    blockPaint
                );
    ...
    )
    {
        ...
        foreach (var seg in segments)
        {
            ...
            var lxMax = Math.Clamp(
                xMax - maxLw,
                channelX + 2f,
                channelX + opts.ChannelWidth - maxLw - 2f
            );
    ...
    private static void DrawBoundaryLabel(
        SKCanvas canvas,
        string text,
        float lx,
        float lineY,
