...
internal static class EventFit
{
    ...
    private static List<Kpc.Event<T>> EventListFitCore<T>(
        List<Kpc.Event<T>>? events,
        double tolerance,
        int? maxDegreeOfParallelism,
        CancellationToken cancellationToken)
    {
        ...
        KpcToolLog.OnInfo($"EventListFit: 开始拟合，共 {events.Count} 个事件，容差={tolerance}% ，并行度={degree}");
        ...
        var outputs = new List<Kpc.Event<T>>[units.Count];
        ...
        var result = new List<Kpc.Event<T>>(sortedEvents.Count);
        foreach (var t in outputs)
            result.AddRange(t);

        KpcToolLog.OnInfo($"EventListFit: 拟合完成，{events.Count} -> {result.Count} 个事件");
        return result;
    }

    private static List<FitUnit> BuildFitUnits<T>(List<Kpc.Event<T>> sortedEvents, double tolerance)
    {
    ...
    /// </summary>
    private static List<Kpc.Event<T>> FitLinearRun<T>(
        List<Kpc.Event<T>> runEvents,
        int startIndex,
        int endExclusive,
        double tolerance,
        int degree,
        CancellationToken cancellationToken)
    {
        ...
        var chosen = new Kpc.Event<T>?[runLength + 1];
        ...
        var reversed = new List<Kpc.Event<T>>();
    ...
    private static Kpc.Event<T> CreateCandidateEvent<T>(Kpc.Event<T> first, Kpc.Event<T> last, int easingId)
        => new()
        {
    ...
    private static bool CanParticipateInFit<T>(Kpc.Event<T> evt)
        => evt.EndBeat > evt.StartBeat && !evt.IsBezier && evt.Easing == 1;

    private static bool CanAppendToFitRun<T>(
        Kpc.Event<T> previousEvent,
        Kpc.Event<T> currentEvent,
        double tolerance)
    {
    ...
    private readonly record struct FittedSegment<T>(Kpc.Event<T> Event, double Score);

    private readonly record struct PlanChoice<T>(double Cost, int Segments, int Prev, Kpc.Event<T> Event);
